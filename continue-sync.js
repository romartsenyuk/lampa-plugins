(function () {
    ...
    var LampaSync = {
        ...
        sync: function (silent) {
            ...
            if (!silent) Lampa.Noty.show('Синхронізація...');
            $.ajax({
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.record && res.record.backup) {
                        // КЛЮЧОВИЙ МОМЕНТ ДЛЯ KINOSMOBILE:
                        // Використовуємо внутрішній метод Lampa для запису з оповіщенням системи
                        Lampa.Storage.set('continue', res.record.backup);
                        
                        // Примусово оновлюємо кеш переглядів у самому додатку
                        ...
                        if (!silent) {
                            ...
                            // Повне перезавантаження для KinosMobile
